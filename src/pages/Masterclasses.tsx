...
import { PlayCircle, Clock, ChevronLeft, ChevronRight, Video, Search, X, Filter } from 'lucide-react';
...
export function Masterclasses() {
  ...
  const filteredMasterclasses = useMemo(() => {
    return filterByLanguage(courses, language)
      ...
      .reverse(); // Organic from new to last
  ...
  return (
    <div className="w-full px-4 md:px-8 py-8 max-w-[1600px] mx-auto">
      ...
      <div className="lg:hidden flex items-center justify-between mb-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('masterclasses') || 'Masterclasses'}</h1>
        </div>
        <button 
      ...
      <div className="flex-1 min-w-0">
        <div className="hidden lg:block mb-8">
          <h1 className="text-4xl md:text-5xl font-black mb-3 tracking-tight">{t('masterclasses') || 'Masterclasses'}</h1>
          <p className="text-muted-foreground text-lg mb-8 max-w-2xl">{t('single_video_courses') || 'Long-form single video courses.'}</p>
        </div>
          ...
          >
            <div className="relative aspect-video overflow-hidden bg-muted">
              ...
              <div className="absolute top-3 start-3 flex flex-wrap gap-2 pe-16 bg-gradient-to-b from-black/60 to-transparent w-full p-3 pt-0 ps-0 start-3">
                <div className="bg-background/95 backdrop-blur text-foreground px-2.5 py-1 rounded-md text-xs font-bold shadow-sm uppercase tracking-wider">
                  {course.category}
                </div>
                {course.subCategory && (
                  <div className="bg-primary/95 backdrop-blur text-primary-foreground px-2.5 py-1 rounded-md text-xs font-bold shadow-sm uppercase tracking-wider">
                    {course.subCategory}
                  </div>
                )}
              </div>
        ...
        {filteredMasterclasses.length === 0 && (
          <div className="col-span-full py-20 flex flex-col items-center justify-center text-center bg-card border border-dashed border-border rounded-3xl">
            ...
            <h3 className="text-xl font-bold mb-2">{t('no_masterclasses_found', 'No masterclasses found')}</h3>
